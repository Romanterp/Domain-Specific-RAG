...
WHY THIS EXISTS
---------------
The RQ3 headline compares CTI between retrieval conditions, but each condition
generated its OWN greedy answer, so the comparison varies the answer text and
the context together. EXPERIMENTS.md carries this as an honest limitation
("a teacher-forced same-answer sensitivity check is possible ... but not yet
run"). This script runs it: every eligible question's DENSE answer and its
HYBRID answer are each re-scored under BOTH contexts, holding the answer fixed
so a within-row contrast varies the context alone.

    X[a,c] = claim-mean CTI of answer a under context c,  a,c in {d, h}
    ...
    S = Delta + A exactly, so the analysis reports what SHARE of the published
    +0.117 interaction was context and what share was answer text.

WHAT THIS CHECK CAN AND CANNOT CLAIM
------------------------------------
The answer is a deterministic function of the context (A_h = greedy(C_h)), so
the row factor is post-treatment and endogenous. "Answer held fixed" is not
"answer exogenous". The legitimate claim is a DESCRIPTIVE DECOMPOSITION of an
observed difference into context and answer components — never an unconfounded
causal estimate of retrieval quality on reliance.

Two confounds are measured rather than assumed, and both are written into every
record so the analysis can adjust for them:

  * COPY CIRCULARITY. CTI rises with how much an answer copies its own context
    (~+0.65 nats per unit 3-gram coverage). The hybrid-answer row is therefore
    partly self-confirming: rescued own-minus-foreign coverage is ~+0.16 there,
    a copy-only budget of roughly +0.04..+0.11 against a published interaction
    of +0.117. The DENSE-answer row is the clean one — copy-only predicts about
    -0.02 for it — so the analysis leads on that row and labels the other.
    Per-cell ngram coverage is recorded here to make the adjustment possible.

  * CITATION MIS-INDEXING. ~44% of answers contain literal [P#] tags and ~78% of
    those tags point at a different chunk once the context is swapped. The
    artifact loads on the home-field term and only approximately cancels from
    the symmetric main effects, so tag counts and swap-safety are recorded per
    cell for the required stratified robustness row.

Refusals are excluded pairwise (a question enters only if BOTH its answers are
non-refusal), which reproduces the published analysis sample exactly: N=696
(468 control + 228 rescued), diagonal targets 1.6545 / 1.6642 / 1.2878 / 1.4147
and interaction +0.1172. That exclusion is differential (it drops all 54
refusal-cured pairs, the strongest effect in the data), so a POSITIVE result is
conservative and usable while a NULL is NOT decisive — the retained rescued
questions are exactly those where dense retrieval was already adequate.
`--arm refusal` spends those excluded pairs instead: it forces the dense REFUSAL
text under both contexts, which contains no gold content to copy and is
therefore structurally immune to the copy circularity above.

VALIDATION BY CONSTRUCTION
--------------------------
Contexts are rebuilt offline from the stored passage_chunk_ids (never
re-retrieved — the candidate-pool default moved 50 -> 100 after the production
run, so re-retrieval would silently return different passages). The diagonal
cells therefore recompute a number that already exists, and agreement with the
recorded answer_cti_mean validates passage rehydration, tokenisation, prompt
format and aggregation in one shot. Run the diagonal first; treat a systematic
miss as a reconstruction failure, not a finding.
